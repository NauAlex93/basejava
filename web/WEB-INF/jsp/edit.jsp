        ...
        <c:forEach var="type" items="<%=SectionType.values()%>">
            ...
            <c:choose>
                ...
                <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
                    <c:forEach var="career" items="<%=((CareerSection) section).getWorkPlaces()%>"
                               varStatus="counter">
                        <dl>
                            <dt>Название учереждения:</dt>
                            <dd><input type="text" name='${type}' size=100 value="${career.link.name}"></dd>
                        </dl>
                        <dl>
                            <dt>Сайт учереждения:</dt>
                            <dd><input type="text" name='${type}url' size=100 value="${career.link.url}"></dd>
                            </dd>
                        </dl>
                        <br>
                        <div style="margin-left: 30px">
                            <c:forEach var="pos" items="${career.positions}">
                                <jsp:useBean id="pos" type="ru.javawebinar.basejava.model.Career.Position"/>
                                <dl>
